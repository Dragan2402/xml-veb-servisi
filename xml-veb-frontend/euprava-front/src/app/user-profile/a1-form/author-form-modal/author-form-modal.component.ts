...
export class AuthorFormModalComponent implements OnInit {
  ...
  private GetAliveAuthorPerson():Person{
    const address = new Address(<string>this.authorPlace.value,<string>this.authorZipCode.value?.toString(),<string>this.authorStreet.value,<number>this.authorStreetNumber.value);
    const citizenship = this.authorIsForeignCitizenship ? new ForeignCitizenship(<string>this.authorPassport.value): new DomesticCitizenship(<string>this.authorJmbg.value);
    return new Person(<string>this.authorFirstName.value,<string>this.authorLastName.value,address,citizenship);
  }
}
